...
namespace utils::publisher
{
     ...
     */
    [[nodiscard]] inline srt::PublishPolicy policy_from_ms(std::uint32_t min_publish_ms) noexcept
    {
     ...
     * @note This overload disables the minimum publish interval (heartbeat) by
     *       passing 0 ms internally. It is intended for publishers that use
     *       AlwaysPublish or otherwise publish on every cycle, where a heartbeat
     *       would be redundant.
     *
     * @tparam FrameT Snapshot frame type.
     * @tparam ReaderT Reader adapter type.
     ...
     * @note This overload avoids a dedicated Reader type by using C-style callbacks.
     *       SnapshotRTOS wraps the callbacks into an internal Reader adapter.
     *
     * @tparam FrameT Snapshot frame type.
     * @tparam PublishT Publish decision type (AlwaysPublish, PublishOnChange, etc.).
     * @param bus Reference to the SnapshotBus to publish frames into.
                         ...
                         BaseType_t core_id = tskNO_AFFINITY) noexcept
    {
        srt::start_frame_publisher_cb<FrameT>(
            ...
            policy_from_ms(min_interval_ms),
            task_name,
            static_cast<std::uint16_t>(stack_words),
            priority,
            period_ms,
            core_id);
    }
