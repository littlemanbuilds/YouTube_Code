 ...
 * MIT License
 *
 * @brief Periodic RTOS diagnostics for heap and registered-task stack high-water marks.
 * @note Only SnapshotRTOS-registered tasks are reported; system tasks are excluded.
 *
 * @file StackWatch.h
...
namespace utils::stack_watch
{
    ...
    namespace detail
    {
        // ---- Local policy (line-specific warnings) ---- //
        constexpr uint32_t kWarnPct = 80;
        constexpr uint32_t kCritPct = 90;

        // ---- Column layout ---- //
        // Keep these consistent across header + rows.
        ...
        /**
         * @brief Compute used percent (rounded) from used/alloc (bytes).
         */
        ...
        /**
         * @brief Print the table header (registry-only).
         */
        ...
        /**
         * @brief Print one registry task row.
         */
        ...
        /**
         * @brief Print registered tasks using the SnapshotRTOS task registry.
